 ...
 * @return decoded in js object structure */
Ns.Libs.SMFreader = function(buffer)
{
    var midiFile = {};
    ...
    function decodeTrackEvent(data, track, trackOffset) {
        ...
        if (i == 0xff)
            ...
            error("Running mode event with no previous event!");
            return -1;
        }
    ...
    function decodeTrack(data, track, trackOffset) {
        ...
        while (idx < end) {
            ...
            if (idx == -1) {
                error("error decoding track event");
                return -1;
            }
        }
        ...
        if (+idx !== +end)
        {
            // TODO: в крайнем случае можно как лучший специалист "idx = end"
            error('Some Events were read incorrectly, pointer is [' + (idx - end) + '] ahead of where it is supposed to be');
            return -1;
        }
...
//   unsigned short ticksPerBeat;
        midiFile.ticksPerBeat = data.getUint16(idx += 2 - 2);
        idx += 2;
